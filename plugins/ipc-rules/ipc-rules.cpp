    ...
    public wf::ipc_rules_events_methods_t
{
  public:
    ...
    wf::ipc::method_callback list_views = [=] (Json::Value)
    {
        Json::Value response = Json::arrayValue;
        for (auto& view : wf::get_core().get_all_views())
        {
            Json::Value v = view_to_json(view);
            response.append(v);
        }
    ...
    wf::ipc::method_callback get_focused_view = [=] (Json::Value data)
    {
        ...
        {
            auto response = wf::ipc::json_ok();
            response["info"] = Json::nullValue;
            return response;
        }
    };

    wf::ipc::method_callback get_focused_output = [=] (Json::Value data)
    {
    ...
    wf::ipc::method_callback focus_view = [=] (Json::Value data)
    {
        auto id = wf::ipc::json_get_uint64(data, "id");
        if (auto view = wf::ipc::find_view_by_id(id))
        {
    ...
    wf::ipc::method_callback close_view = [=] (Json::Value data)
    {
        auto id = wf::ipc::json_get_uint64(data, "id");
        if (auto view = wf::ipc::find_view_by_id(id))
        {
    ...
    wf::ipc::method_callback list_outputs = [=] (Json::Value)
    {
        Json::Value response = Json::arrayValue;
    ...
    wf::ipc::method_callback configure_view = [=] (Json::Value data)
    {
        auto id = wf::ipc::json_get_uint64(data, "id");
        auto output_id = wf::ipc::json_get_optional_uint64(data, "output_id");

        if (data.isMember("geometry") && !data["geometry"].isObject())
        {
        ...
        if (output_id.has_value())
        {
            ...
            wf::move_view_to_output(toplevel, wo, !data.isMember("geometry"));
        }

        if (data.isMember("geometry"))
        {
    ...
    wf::ipc::method_callback list_wsets = [=] (Json::Value)
    {
        Json::Value response = Json::arrayValue;
    ...
    wf::ipc::method_callback get_wset_info = [=] (Json::Value data)
    {
        auto id = wf::ipc::json_get_uint64(data, "id");
